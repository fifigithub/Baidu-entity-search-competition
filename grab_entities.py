...
gflags.DEFINE_string("db_loc", "entities_db/entities.db", "When set, will only retry on failed entities.")
...
class WritingThread(threading.Thread):
    ...
    def run(self):
        ...
        cursor = conn.cursor()
        if not self.modify:
            cursor.execute('create table search_results(entity_name text, response text, failed bool)')
        ...
        while n_finished < self.len_all_entities:
            ...
            if not self.modify or not EntityNameAlreadyOccur(entity_name):
                cursor.execute('insert into search_results values (?, ?, ?)', (entity_name, text, failed))
            else:
                cursor.execute('update search_results set response=?,failed=? where entity_name=?', (
...
def GetFailedEntities(loc):
    ...
    cursor = conn.cursor()
    successful_entities = [i[0] for i in cursor.execute('select entity_name from search_results where not failed')]
